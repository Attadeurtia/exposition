...
import introMarkdown from '@/assets/Duchesses/Jeanne de Penthièvre/introduction.md';

export default {
  name: 'JeanneDePenthievre',
  components: {
    MapComponent
  },
  data() {
  ...
  methods: {
    ...
    async loadMarkdownTexts() {
      ...
      await Promise.all(
        this.images.map(async (image) => {
          try {
            const response = await import(`@/assets/Duchesses/Jeanne de Penthièvre/${image.id} cartel.md`);
    ...
    async loadJustificationTexts() {
      ...
      await Promise.all(
        this.images.map(async (image) => {
          try {
            const response = await import(`@/assets/Duchesses/Jeanne de Penthièvre/${image.id} justification.md`);
